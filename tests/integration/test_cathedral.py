...
@pytest.mark.asyncio
async def test_cathedral_interface_with_mock_agent():
    """
    ...
    async def mock_spawn_agent(agent_type, channel, bus_url):
        ...
        event_to_send = {
            ...
            "payload": {"result": "Mock Agent Completed Vision"},
        }
        await mock_agent_khala.send(event_to_send)
        await mock_agent_khala.disconnect()

    with patch.object(
        gateway, "spawn_agent", new_callable=AsyncMock, side_effect=mock_spawn_agent
    ) as mock_spawn:
        async with Protoss(vision, port=0, coordination_id=coordination_id) as swarm:
            ...
            assert any(e["type"] == "coordination_complete" for e in received_events)

            completion_event = next(
                e for e in received_events if e["type"] == "coordination_complete"
            )
            assert (
                completion_event["payload"]["result"] == "Mock Agent Completed Vision"
            )
...
@pytest.mark.asyncio
async def test_cathedral_event_streaming():
    """
        ...
        await mock_agent_khala.send(
            {
                ...
                "type": "agent_spawn",
                "coordination_id": coordination_id,
                "payload": {"agent_id": f"{agent_type}-stream-mock"},
            }
        )
        ...
        await mock_agent_khala.send(
            {
                ...
                "payload": {"thought": "complex calculation"},
            }
        )
        ...
        await mock_agent_khala.send(
            {
                ...
                "type": "coordination_complete",
                "coordination_id": coordination_id,
                "payload": {"result": "Streaming complete"},
            }
        )
        await mock_agent_khala.disconnect()
    ...
    with patch.object(
        ...
        assert (
            next(e for e in received_events if e["type"] == "agent_message")["content"]
            == "Thinking hard..."
        )
        assert (
            next(e for e in received_events if e["type"] == "coordination_complete")[
                "payload"
            ]["result"]
            == "Streaming complete"
        )
