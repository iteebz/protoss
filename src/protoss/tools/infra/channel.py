...
class ChannelCreate(Tool):
    ...
    schema = {
        ...
        "description": {"type": "string", "optional": True, "description": "Optional channel description"}
    }
    ...
    async def execute(self, channel: str, description: str = None, **kwargs) -> ToolResult:
        if not channel:
            return ToolResult(outcome="Channel name cannot be empty")
        
        if not channel.replace("_", "").replace("-", "").isalnum():
            return ToolResult(outcome="Channel name must be alphanumeric with underscores/hyphens")
        ...
        try:
            ...
            async with websockets.connect(bus_url) as websocket:
                ...
                if result.get("success"):
                    ...
                    return ToolResult(outcome=f"Failed to create channel: {result.get('error', 'Unknown error')}")
            ...
            async with websockets.connect(bus_url) as websocket:
                ...
                if result.get("success"):
                    ...
                    if channels:
                        ...
                        return ToolResult(outcome=f"Found {len(channels)} channels", content=channel_info)
                    else:
                        return ToolResult(outcome="No channels found")
                else:
                    return ToolResult(outcome=f"Failed to list channels: {result.get('error', 'Unknown error')}")
