...
class AgentSpawn(Tool):
    ...
    schema = {
        "agent_type": {"type": "string", "description": "Type of agent to spawn (arbiter, probe, zealot, etc)"},
        "channel": {"type": "string", "description": "Channel to spawn agent in"},
        "coordination_id": {"type": "string", "optional": True, "description": "Coordination ID for task context"}
    }
    ...
    async def execute(self, agent_type: str, channel: str, coordination_id: str = None, **kwargs) -> ToolResult:
        if not agent_type:
        ...
        try:
            ...
            if not config_path.exists():
                return ToolResult(outcome="Protoss not configured - run 'protoss config'")
            ...
            async with websockets.connect(bus_url) as websocket:
                ...
                if result.get("success"):
                    ...
                    return ToolResult(outcome=f"Failed to spawn agent: {result.get('error', 'Unknown error')}")
...
class AgentList(Tool):
    ...
    schema = {
        "channel": {"type": "string", "optional": True, "description": "Filter by specific channel"}
    }
            ...
            async with websockets.connect(bus_url) as websocket:
                ...
                if result.get("success"):
                    ...
                    if agents:
                        ...
                        return ToolResult(outcome=f"Found {len(agents)} active agents", content=agent_info)
                    else:
                        return ToolResult(outcome="No active agents found")
                else:
                    return ToolResult(outcome=f"Failed to list agents: {result.get('error', 'Unknown error')}")
