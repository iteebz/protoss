...
class Zeratul(Unit):
    ...
    @property
    def tools(self):
        """Pure constitutional reasoning - severed from external dependencies."""
        return []

    # IMPLEMENTATION - Coordination methods
    async def execute(self, task: str, pathway: str) -> str:
        """Execute shadow investigation."""
        print(f"👤 {self.id} investigating: {task[:50]}...")
