...
async def trial_by_fire():
    ...
    print(f"🔥 TRIAL BY FIRE: Team Coordination")
...
""")
    ...
    while time.time() - start_time < timeout:
        ...
        conversation = protoss.get_conversation(channel)
    ...
    # Initialize last_activity_time if needed
    if 'last_activity_time' not in locals():
    ...
    print(f"Recent coordination:")
    print()
    
    for msg in conversation[-6:]:
        sender = msg['sender'].upper() if msg['sender'] != 'human' else 'HUMAN'
        content = msg['content'][:160] + "..." if len(msg['content']) > 160 else msg['content']
        print(f"{sender}: {content}")
        print()
    
    # Check coordination patterns
    claims = [msg for msg in conversation if "i'll handle" in msg['content'].lower() or "i'll take" in msg['content'].lower()]
    reports = [msg for msg in conversation if "completed" in msg['content'].lower() or "finished" in msg['content'].lower()]
    human_asks = [msg for msg in conversation if any(phrase in msg['content'].lower() for phrase in ['please try', 'please run', 'please navigate'])]
    team_help = [msg for msg in conversation if any(phrase in msg['content'].lower() for phrase in ['any ideas', 'getting error', 'teammates'])]
